...
4. **Create a Google OAuth client** (one-time):
   ...
   5. Download the JSON and save it as `credentials.json` in the
      `nvoids-job-agent/` folder (next to `requirements.txt`), or set
      `CREDENTIALS_PATH` in `.env` to an absolute path.
